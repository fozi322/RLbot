...
class Nexto(BaseAgent):
    ...
    def perform_speedflip(self):
        # Define the sequence of actions for a speedflip
        actions = [
            SimpleControllerState(throttle=1, jump=True, pitch=-1),  # Initial jump while pitching back
            SimpleControllerState(throttle=1, jump=False, pitch=-1, yaw=-1),  # Diagonal flip to the left
            SimpleControllerState(jump=True, pitch=1, boost=True)  # Cancel the flip and boost
        ]
    ...
    def maybe_do_kickoff(self, packet, ticks_elapsed):
        if packet.game_info.is_kickoff_pause:
            # Recarregar variáveis de kickoff
            self.load_kickoff_controls()
    ...
    def load_kickoff_controls(self):
        with open("kickoff.txt", "r") as f:
            file_content = f.read()
